...
import { prettyDate, calculateAge } from "../utils/FormatDate";

export default function RouteDataComponent({ data }) {
    return (
        <Link href={`/trips/${data.trip_id}`} asChild>
            <Pressable style={styles.routeDataContainer}>
                ...
                <View style={[styles.hourInfo, styles.info]}>
                    ...
                    <Text style={styles.infoText}>{prettyDate(data.date_time_departure)}</Text>
                </View>
                ...
                <View style={[styles.ratingInfo, styles.info]}>
                    ...
                    <Text style={styles.infoText}>{data.Rating[0] ? calculateRating(data.Rating).average : ('0.0')} / 5.0 - {calculateRating(data.Rating).opinions} opiniones</Text>
                </View>
                <View style={styles.driverInfoSection}>
                    ...
                    <View style={styles.driverInfo}>
                        <Text style={styles.driverNames}>{data.Profile.Person.person_name.split(' ')[0]} {data.Profile.Person.person_last_name.split(' ')[0]}</Text>
                        <Text style={styles.driverAge}>{calculateAge(data.Profile.Person.birth_date)} años</Text>
                    </View>
                </View>
